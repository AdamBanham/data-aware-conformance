...
from felli.smt.ysolver import YicesSolver
from felli.smt.z3solver import Z3Solver
# from smt.omsolver import OptiMathsatSolver
#from smt.cvc5solver import CVC5Solver
from felli.dpn.read import read_json_input, read_pnml_input
from felli.cluster.partitioning import NaivePartitioning, IntervalPartitioning
from felli.dpn.dpn import DPN
# from dpn.playout import Playout, traces_to_xes
from felli.encoding.encoding import Encoding
from felli.encoding.encoding_exhaustive import ExhaustiveEncoding
from felli.dpn.expr import Expr
import felli.uncertainty.read
from felli.uncertainty.encoding import UncertaintyEncoding
from felli.uncertainty.trace import UncertainTrace, UncertainLog, UncertainDataValue
from felli.uncertainty.uncertainize import all as uncertainize_all, extending as uncertainty_extending
from felli.utils import pad_to, spaces
from felli.options import default as default_options
...
def conformance_check_aggregated(log, dpn, verbose, anti):
  logs = preprocess_log(log, dpn)
  tracepart = NaivePartitioning([(log,1) for log in logs])
...
def guard_recall(dpn:str, log) -> float:
  """
  The proposed guard-recall measure from cocomot, where we take a negatively
  weighted average sum of distances for equivalent classes. 

  Params
  ------
  dpn:`str` - a filepath to the model in question
  log:`str` - a filepath to the log in question

  Returns
  -------
  `float` - A weighted average of the distances between classes
  """
  import yices_api as yapi
  yapi.yices_init()
  # preprocess in the same manner
  dpn = DPN(read_pnml_input(dpn))
  (log, has_uncertainty) = read_log(log)
  # use some default opts to simplify things.
  opts = {
    'verbose' : 0,
    'numprocs': 1,
    "many" : None,
