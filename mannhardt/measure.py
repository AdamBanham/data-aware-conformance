...
def call_java(log, model, type) -> Dict[str,str]:
    """
...
def guard_recall(logfile:str, modelfile:str) -> float:
    """
    Computes the recall notion from "Balanced multi-perspective checking of 
    process conformance" by F. Mannhardt et. al. in 2015.
    ...
    run_data = call_java(logfile,modelfile, "MannhardtRecall")
    return float(run_data["mean"])

def guard_precision(logfile:str, modelfile:str) -> float:
    """
    Computes the precision notion from "Measuring the Precision of Multi-perspective
    Process Models" by F. Mannhardt et. al. in 2016.
    ...
    if not test_for_executables():
        raise Exception(
            "Unable to compute recall notion as PromExecutables"+
            "have not been compiled, see java directory to compile.")
    run_data = call_java(logfile,modelfile, "MannhardtPrecision")
    return float(run_data["mean"])