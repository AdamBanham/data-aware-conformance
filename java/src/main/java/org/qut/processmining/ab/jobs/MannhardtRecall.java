...
public class MannhardtRecall {
	
	public static int defaultMoveOnModelCost = 2;
	public static int defaultMoveOnLogCost = 3;
	public static int defaultMissingWriteOpCost = 1;
	public static int defaultIncorrectWriteOpCost = 1;
	public static long configMaxTraceCompute = 180; // in seconds, == 5 minutes max per trace.
	...
	public static void main(HashMap<String,String> args) throws Throwable {
		...
		try {
			...
			System.out.println("[Job] starting job :: deLeoni");
			...
			if (args.containsKey("model")) {
				...
				runBalancedConformance(args, args.get("model"), outputcsvWriter,null);
	...
	public static void runBalancedConformance(HashMap<String,String> args, String inputModel, FileWriter outputcsv, File outputlog) throws Throwable {
		try {
			System.out.println("[Job] starting job :: deLeoni :: "+args.get("model"));
			// load in the .xes.gz log 
			Collection<XLog> logs = new ArrayList<XLog>();
			try {
				System.out.println("[Job] parsing log...");
				logs = new XesXmlParser().parse(new File(args.get("log")));
			} catch (Exception ie) {
